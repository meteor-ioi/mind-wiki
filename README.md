...
- 🔗 **知识库关联**：通过 `LINKS.md` 建立网状知识关联，支持跨库查询（如：反思层引用系统层）
...
- [[另一个知识库名称]]
```
此时查询当前库时，AI 将自动获得查看关联库的授权，实现知识的网状贯通。

## 示例 Demo (最佳实践)
项目根目录下提供了两个分层架构的 Demo 供参考：
- **`01-立体成长系统示例`**：展示个人如何从价值观到方法论，再到行动反思的闭环。
- **`02-企业岗位体系示例`**：展示企业如何将文化、部门 SOP 和具体岗位思维进行数字化装载。

这些示例展示了 `LINKS.md` 如何在不同层级间传递决策逻辑。
